    ...
    <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
      ...
      <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
        <div className="flex justify-between items-center">
          ...
          <p className="text-gray-800 font-medium mt-8 px-8 ">
            {filteredJobs.length > 0
              ? `${(currentPage - 1) * 6 + 1} - ${Math.min(currentPage * 6, filteredJobs.length)} of ${filteredJobs.length} Jobs`
              : "No Jobs Found"}
          </p>
        </div>
        ...
        {/* Pagination - Fixed anchor tag issues */}
        {filteredJobs.length > 0 && (
          <div className='flex items-center justify-center space-x-2 mt-10'>
            ...
            <img
              ...
              alt="Next Page"
              className={`cursor-pointer ${currentPage === Math.ceil(filteredJobs.length / 6) ? 'opacity-50 pointer-events-none' : ''}`}
            />
          </div>
        )}
      </section>
